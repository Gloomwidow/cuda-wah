...
{
    if (get_segmentlen_inblock == GET_SEGMENTLEN_INBLOCK_SOA)
        return get_segmentlen_inblock_soa_sync(is_begin, w_type, smem_ptr, lane_id, warp_id, warps_count);
    else
        return get_segmentlen_inblock_aos_sync(is_begin, w_type, smem_ptr, lane_id, warp_id, warps_count);
}
