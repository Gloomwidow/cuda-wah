...
{
	if (calc_segmentlen_ingrid == CALC_SEGMENTLEN_INGRID_NONCOALESCED)
		calc_segmentlen_ingrid_noncoalesced_sync(segment_len, index, is_begin, w_type, smem_ptr, gmem_ptr, lane_id, warp_id, warps_count, grid);
	else
		calc_segmentlen_ingrid_coalesced_sync(segment_len, index, is_begin, w_type, smem_ptr, gmem_ptr, lane_id, warp_id, warps_count, grid);
}
...
__device__ void calc_segmentlen_ingrid_coalesced_sync(int &segment_len, int &index, bool &is_begin, WORD_TYPE w_type, void* smem_ptr, void* gmem_ptr, int lane_id, int warp_id, int warps_count, cg::grid_group grid)
{
	...
	{
		if (blockIdx.x > 0 && threadIdx.x == 0 && w_type != TAIL_WORD &&								// check if the first thread-beginning in block is really
			g_block_segments.r_end_type[blockIdx.x - 1] == w_type)  										// thread-beginning in the context of the grid...
		{
			is_begin = false;
			am_last_beginning_inblock = false;
			*s_decrement_index = true;
