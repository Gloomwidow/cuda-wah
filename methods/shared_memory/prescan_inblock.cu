...
{
	switch (prescan_inblock)
	{
		case PRESCAN_WORKEFFICIENT:
			prescan_inblock_workefficient_sync(value, smem_ptr, lane_id, warp_id, warps_count);
			break;
		case PRESCAN_WORKEFFICIENT_CONFLICTFREE:
			prescan_inblock_workefficient_conflictfree_sync(value, smem_ptr, lane_id, warp_id, warps_count);
			break;
#ifdef UNROLLED_PRESCANS
		case PRESCAN_SHFL_UNROLLED:
			prescan_inblock_shfl_unrolled_sync(value, smem_ptr, lane_id, warp_id, warps_count);
			break;
		case PRESCAN_WORKEFFICIENT_UNROLLED:
			prescan_inblock_workefficient_unrolled_sync(value, smem_ptr, lane_id, warp_id, warps_count);
			break;
		case PRESCAN_WORKEFFICIENT_CONFLICTFREE_UNROLLED:
			prescan_inblock_workefficient_conflictfree_unrolled_sync(value, smem_ptr, lane_id, warp_id, warps_count);
			break;
#endif
		default:
			prescan_inblock_shfl_sync(value, smem_ptr, lane_id, warp_id, warps_count);
	}
}
...
__device__ void prescan_inblock_workefficient_sync(int &value, void* smem_ptr, int lane_id, int warp_id, int warps_count)
{
	...
	value = s_values[threadIdx.x] + value;
	__syncthreads();
}
...
__device__ void prescan_inblock_workefficient_conflictfree_sync(int &value, void* smem_ptr, int lane_id, int warp_id, int warps_count)
{
	...
	// down-sweep
	if (threadIdx.x == 0)
		s_values[blockDim.x - 1 + CONFLICT_FREE_OFFSET(blockDim.x-1)] = 0;
	for (int d = 1; d < blockDim.x; d *= 2)
	{
		offset >>= 1;
		__syncthreads();
...
#ifdef UNROLLED_PRESCANS
// taken from https://github.com/NVIDIA/cuda-samples/blob/master/Samples/shfl_scan/shfl_scan.cu
__device__ void prescan_inblock_shfl_unrolled_sync(int &value, void* smem_ptr, int lane_id, int warp_id, int warps_count) // niepotrzebny argument
{
	int* s_sums = (int*)smem_ptr;
